...
import { getSellerReportDetails } from "./req-res";
import type { SellerReportDetails } from "./interfaces";
...
export default function SellerReportDetailsPage() {
  const params = useParams<{ reportId: string }>();
  const reportId = params?.reportId ?? "";

  const [report, setReport] = useState<SellerReportDetails | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const data = await getSellerReportDetails(reportId);
        if (!mounted) return;
  ...
  return (
    <div className={styles.page}>
      <Link href="/orders-to-sell" className={styles.back}>
        ← Back to sell orders
      </Link>

      <section className={styles.header}>
        <h1 className={styles.title}>Report About Your Order</h1>
        <p className={styles.subtitle}>Victim-side view. Report ID: {reportId || "-"}</p>
      </section>
      ...
      {!loading && !error && report && (
        <>
          ...
          <section className={styles.panel}>
            <h2 className={styles.panelTitle}>What happened</h2>
            <p className={styles.text}>{report.description || "-"}</p>
          </section>

          <section className={styles.panel}>
            <h2 className={styles.panelTitle}>People Involved</h2>
            <div className={styles.grid}>
              ...
              <div className={styles.field}>
                <span className={styles.label}>Reported User (You)</span>
          ...
          <section className={styles.panel}>
            <h2 className={styles.panelTitle}>Evidence Shared</h2>
