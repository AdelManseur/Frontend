...
export async function markMessageAsRead(messageId: string): Promise<{ messageId: string; status: string }> {
  ...
  if (!res.ok) {
    throw new Error(data?.message || `Failed to mark message as read (${res.status})`);
  }
